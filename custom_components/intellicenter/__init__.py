...
PLATFORMS = [LIGHT_DOMAIN, SENSOR_DOMAIN, SWITCH_DOMAIN, BINARY_SENSOR_DOMAIN, WATER_HEATER_DOMAIN]
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    filterFunc = lambda object: object.status and object.objtype in ['BODY','SENSE','PUMP','HEATER','CIRCUIT']
    ...
    class Handler(ConnectionHandler):
        ...
        def disconnected(self, controller, exc):
            ...
            _LOGGER.info(f"disconnected from system: '{controller.systemInfo.propName}'")
            dispatcher.async_dispatcher_send(
                hass,
                DOMAIN + "_DISCONNECT_" + entry.entry_id
            )   
...
class PoolEntity(Entity):
    ...
    async def async_added_to_hass(self):
        self.async_on_remove(
            dispatcher.async_dispatcher_connect(
                self.hass,
                DOMAIN + "_UPDATE_" + self._entry_id,
    ...
    def requestChanges(self, changes: dict) -> None:
        """ request changes as key:value pairs on the associated Pool object """
        ...
        self._controller.requestChanges(self._poolObject.objnam,changes,waitForResponse = False)

    @callback
    def _update_callback(self, changes):
        """Update the the entity if its underlying pool object has changed"""
        for object in changes:
    ...
    def _disconnect_callback(self):
        """ Mark the entity as unavailble since we got disconnected from the server """
